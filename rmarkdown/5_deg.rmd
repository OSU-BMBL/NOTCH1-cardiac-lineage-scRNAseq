...
dir.create(paste0("../deg/day", day))
RESULT_DIR <- paste0("../deg/day", day, "/")
...
for (i in seq_along(this_cell_type)) {
  ...
  if (length(this_up) > 10) {
    ...
    write.csv(
      ...
      paste0(RESULT_DIR, this_ct, "GO_MF_up.csv")
    )
    write.csv(
      enriched_combined$GO_Cellular_Component_2018,
      paste0(RESULT_DIR, this_ct, "GO_CC_up.csv")
    )
    write.csv(
      enriched_combined$GO_Biological_Process_2018,
      paste0(RESULT_DIR, this_ct, "GO_BP_up.csv")
    )
    write.csv(enriched_combined$KEGG_2019_Human,
              paste0(RESULT_DIR, this_ct, "KEGG_up.csv"))
  ...
  if (length(this_down) > 10) {
    ...
    write.csv(
      enriched_combined$GO_Molecular_Function_2018,
      paste0(RESULT_DIR, this_ct, "GO_MF_down.csv")
    )
    write.csv(
      enriched_combined$GO_Cellular_Component_2018,
      paste0(RESULT_DIR, this_ct, "GO_CC_down.csv")
    )
    write.csv(
      enriched_combined$GO_Biological_Process_2018,
      paste0(RESULT_DIR, this_ct, "GO_BP_down.csv")
    )
    write.csv(
      enriched_combined$KEGG_2019_Human,
      paste0(RESULT_DIR, this_ct, "KEGG_down.csv")
    )
